...
As Reviewers A and D point out, the delta from the prior POPL 2022 work is moderate
in the core aims and ideas, but lies instead in a redesigned approach to the implementation of the concepts. We
believe the approach proposed provides sufficient benefits to warrant separate study, and have identified
...
2022 work: the Galois connections were of types f: I → O1 and g: I → O2 (where ‘I’ indicates input and ‘O’
...
approach, via a product construction where all the operations have conjugates. (For example the projection π₁: O1 × O2 → O1 has a
conjugate [id , const ⊥] : O1 → O1 × O2 which supplies the bottom demand for the right view.) We will explain
this to clarify the delta from the POPL 2022 work and show how additional general-purpose combinators like π₁ for
operating on data dependencies are easy to derive. We will also show how this provides a formal account of
...
- _Difficult terminology and concepts in first 2 sections._ We are sympathetic and address this in R2
  above.
...
- Expand on differences between our approach vis-a-vis POPL 2022 paper (Reviewers A and D):
  - Explain why POPL 2022 approach doesn’t directly support related inputs, and how that approach can
...
- Mention idea of distinguishing information about choices/control flow from “content” and relate to
