There are 3 comments I found most important to revisit: Reviewer D [Comment 1], Reviewer C [Comment 1 + 2]
...
1. _"we are not aware of such a feature being used to ...."_
   ...
   I'm not sure what single feature we are talking about here? I read 3 features having been listed.
...
**General proposal**: I would also restructure to something like the following (which doesn't address all of my comments above):

C1. _Claims of novelty_. Agreed -- this can be better clarified. While there is prior work on (1) generating
provenance/dependency graphs, (2) brushing and linking over such graphs, and (3) database-based
queries over such graphs, we believe we are the first to:
...
For example, it explains why the same function can be computed in two different ways, with potentially
different performance, via De Morgan duals.
...
**General proposal**:  I would rewrite this to something like:
