...
- [ ] We call this broad idea of [..] _data transparency_, and illustrate this using our implementation in Fig 1.
- [ ] Consider the stacked bar chart on the left of the figure. As with the histogram, there are many questions even an expert reader might have about what is being plotted. For example in renewable energy there is an important distinction between _capacity_ and _output_ -- which is being plotted here? What kinds of energy count as "non-renewable"?
...
- [ ] Although it falls short of providing a full explanation […] this does significantly improve on the current state of affairs because these queries do not have to be anticipated in advance by the author of the visualisation. The infrastructure providing them can be validated once for a given language rather than implemented on an ad hoc basis for each application.
