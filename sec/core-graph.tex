...
Foreign functions must be defined precisely for inputs that are appropriately truncated. For example, a binary operator $*$ with annihilator $0$ must define rules such as:

\begin{salign}
\annot{\exInt{0}}{\alpha} * \beta &\geval \annot{\exInt{0}}{\alpha'} \\
\annot{\exInt{n}}{\alpha} * \annot{\exInt{n}}{\beta} &\geval \annot{(\exInt{n \mathbin{\hat{*}} m})}{\alpha'} \\
\end{salign}
