...
For any constructor $c$ we write $\datatype{\Delta}{c}$ to mean either the datatype that $c$ is a member of
...
can either return an eliminator, or something else. We call an eliminator a \textit{surface eliminator} when it's
output type is a surface expression. We call it a \textit{core eliminator} if it's output type is a core expression.
