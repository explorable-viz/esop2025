...
\todo{Need an intro. And rehash this section so it's more goal-oriented. Currently a bit unfocused.} We
introduce some prelimaries -- Boolean algebras (\defref{conjugate:Boolean-algebra}) and the image and preimage
functions for a relation and their De Morgan duals (\secref{conjugate:image-preimage}) -- and then extend
these operators to dependence graphs (\secref{conjugate:dependence-graph-functions}) via their IO relation,
and give algorithms for computing them over a given graph (\secref{conjugate:dependence-graph-algos}).
\todo{Example needed?}
...
Since $\neg$ for $\powerset{X}$ is just relative complement $X \setminus \param$, this also has an intuitive
reading: $\neg{\demandedByR_{R}(\neg X')}$ contains all the outputs that don't depend on anything outside of
$X'$.
