...
a particular dependency relation $R$. A mnemonic device may help with reading the symbols; if $R$ is a
dependency relation oriented with outputs at the top and inputs at the bottom, then \emph{demanded by}
$\demandedByR$ can be read as an arrow pointing from inputs to outputs, and \emph{demands} $\demandsR$ as an
arrow pointing from outputs to inputs.
...
\textit{conjugate pair}, in the sense of Jonsson and Tarski~\cite{jonsson51}:
...
\noindent (Jonsson and Tarski~\cite{jonsson51} consider only endofunctions; here we extend the idea of
conjugacy to maps between Boolean algebras.)
