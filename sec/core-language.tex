...
does not have an associated address. These addresses, explained in \secref{core:addresses}, will (later)
\todo{weird forward-reference} form the vertices of the graphs that we build in the graph semantics.
...
some freshness condition is satisfied -- \JBnote{ensuring there are no collisions in a value's underlying graph}. For simplicity,
we elide \JBnote{the freshness condition from the graph semantics (next), and simply assume it is satisfied whenever a 
new address $\alpha$ is created.}
