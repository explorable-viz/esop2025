...
A \emph{continuation} $\kappa$  is either a term $e$ or an eliminator $\sigma$, and describes how an execution
...
we find this makes for a cleaner presentation than a single shallow elimination form for each type. Piecewise function definitions in the surface language desugar into eliminators.
...
We define addressed \emph{values} $v$ mutually inductively with \emph{raw values} $\raw{v}$; a raw value
is simply a value without an associated address. \JBnote{Each (partial) value is decorated with an address $\alpha$.
...
long as a freshness condition is satisfied -- ensuring no collisions when considered as vertices in the
values underlying graph. For simplicity, we elide the freshness condition from the graph semantics in
(next), and simply assume it will be satisified whenever a new address $\alpha$ is created.}
