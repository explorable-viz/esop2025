...
An eliminator specifies how a value of a particular type is matched, and for a given value determines how any
pattern variables get bound, and the continuation $\kappa$ which to be executed under those additional
bindings. A variable eliminator $\elimVar{x}{\kappa}$ says how to match any value (as variable $x$) and
continue as $\kappa$. A record eliminator $\elimRecord{\seq{x}}{\kappa}$ says how to match a record with
fields $\seq{x}$, and provides a continuation $\kappa$ for matching the values of those fields. Lastly, the
eliminator $\elimConstr{\seq{c \mapsto \kappa}}$ provides a branch $c \mapsto \kappa$ for each constructor $c$
in $\seq{c}$, where each $\kappa$ specifies how any arguments to $c$  will be matched. (We assume that any
