...
A continuation which is an expression $e$ only matches the empty stack of values; in that case
pattern-matching is complete and $e$ is the selected branch. A variable eliminator $\elimVar{x}{\kappa}$
matches any non-empty stack of values, popping $v$ off the stack and and selecting $\kappa$ as the
...
to be recursively matched by $\sigma$. Specifically, the record eliminator $\elimRecord{\seq{y}}{\kappa}$
matches a record of the form $\annot{\exRec{\seq{\bind{x}{v}}}}{\alpha}$ as long as the variables in $\seq{y}$
are also fields in $\seq{x}$, returning the address $\alpha$ associated with the record; the premise
$\exRec{\seq{\bind{y}{u}}} \subseteq \exRec{\seq{\bind{x}{v}}}$ projects out the corresponding values of
$\seq{u}$ from $\seq{v}$. Lastly, a constructor eliminator with a clause of the form $(\elimBind{c}{\kappa})$
matches any constructor value of the form $\annot{\exConstr{c}{\seq{v}}}{\alpha}$, returning the address
$\alpha$ associated with the constructor.
