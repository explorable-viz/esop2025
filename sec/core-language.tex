...
empty) set of named functions with which $\sigma$ was mutually defined.

An \emph{environment} is a finite map from variables to values. Because foreign functions in the core language
are not first-class and calls $\exForeignApp{f}{\seq{e}}$ are saturated, i.e.~$\length{\seq{e}} = \Phi{(f)}$,
the surface language provides a top-level environment which maps every foreign function ${f}$ of arity $n$ to
the closure
