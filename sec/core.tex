...
\todo{Needs an intro.} We introduce the basic syntactic forms of the language (\secref{core:syntax}) and show
how parts of values in our language are labelled with addresses. Then we define \emph{dynamic dependence
graphs} (\secref{core:dependence-graphs}) give a big-step operational semantics that evaluates a term to
a value and constructs a dependence graph for the evaluation of that term (\secref{core:semantics}). The big
step semantics uses auxiliary definitions that support pattern-matching and mutual recursion.
...
forms for our purposes, e.g.~for matching dictionaries, and cite \citet{peytonjones2021} as precedent here}
that specify how this pattern-matching is done and the subsequent continuation $\kappa$ to be executed under
additional variable bindings. These are what the piecewise function definitions of our surface language
desugar into; \citet{peytonjones2021} present a similar idea.
...
empty) set of named functions with which $\sigma$ was mutually defined. An \emph{environment} is defined as a
...
A \emph{dynamic dependence graph}~\cite{agrawal90} is a directed acyclic graph $G = (V,E)$ with a set $V$ of
\emph{vertices} and a set $E \subseteq V \times V$ of edges. In the dependence graph for a particular program,
vertices $\alpha, \beta \in V$ represent \emph{parts} of values (either supplied to the program as inputs or
