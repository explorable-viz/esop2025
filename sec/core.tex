\section{A Core Calculus for Transparent Research Outputs}
\label{sec:core}

\todo{Needs an intro.} After introducing the basic syntactic forms of the language (\secref{core:syntax}) and
show how parts of values are to be treated as vertices in a dependence graph, we then go on to give a big-step
operational semantics that evaluates a term to a value and constructs a dependence graph for the evaluation of
that term (\secref{core:semantics}). The big step semantics uses auxiliary definitions that support
pattern-matching and mutual recursion.
