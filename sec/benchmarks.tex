...
for backward and forward slicing respectively. The implementations using the
opposite slicing procedure on the opposite graph are denoted as \GraphBwdOp,
and \GraphFwdOp.
\subsection{Old}
...
backward slicing, as \TraceEval,\TraceFwd, and \TraceBwd.
...
to a graph in \GraphEval. This fixes the dependence graph $G:\mathcal{A} \to \mathcal{B}$,
...
\GraphBwd refers to running the backward slicing procedure over a fixed
selection. We use this to compute a relevant input selection as we have described.
The input selection we compute during \GraphBwd  is the selection we then
feed into the forward slicing procedure which we time as part of \GraphFwd.
